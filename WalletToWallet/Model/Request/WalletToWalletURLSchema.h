//
//  WalletToWalletURLSchema.h
//  WalletToWallet
//
//  Created by harish on 02/03/2014.
//  Copyright (c) 2014 Narendra. All rights reserved.
//

#ifndef WalletToWallet_WalletToWalletURLSchema_h
#define WalletToWallet_WalletToWalletURLSchema_h

#define WalletToWallet_URL_BASE    @"http://staging.mavinwallet.com/app.php?method="

#define LOGIN @"login"
#define REGISTRATION @"registration"
#define BALANCE @"mybalance"
#define MY_TRANSECTION @"mytransactions"
#define MY_REWARDS @"myrewards"
#define WALLET_RECHARGE_CHARGES @"wallet_recharge_charges"
#define MY_REWARD_CURRENCUY @"myreward_currency"


//4042014
#define WALLET_TRANSFER_FUND_CHARGES @"wallet_transfer_fund_charges"
#define GET_WALLET_ACCOUNT_NAME @"get_wallet_account_name"
#define MY_ACCOUNT @"myaccount"
#define WALLET_RECHARGE_CONFIRM @"wallet_recharge_confirm"
#define TRANSFER_FUND_CONFIRM @"transfer_fund_confirm"

#define SPECIAL_OFFER @"special_offers"


#define  CONCIERGE @"concierge"
#define  AIRTIME @"airtime"
#define  CHANGE_PASSWORD @"change_password"
#define  PASSWORD_FORGOTTEN @"password_forgotten"
#define  SPECIAL_OFFERS @"special_offers"


#endif
